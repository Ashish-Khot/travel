...
router.get('/direct/:touristId/:guideId', verifyToken, async (req, res) => {
    try {
        const { touristId, guideId } = req.params;
        console.log('[CHAT] GET /direct/:touristId/:guideId', { touristId, guideId, user: req.user });

        // Access control
        if (![touristId, guideId].includes(req.user.userId)) {
            console.log('[CHAT] Access denied', { userId: req.user.userId, touristId, guideId });
            return res.status(403).json({ error: 'Access denied' });
        }
        ...
        console.log('[CHAT] Found chat:', chat);
        ...
        if (!chat) {
            ...
            console.log('[CHAT] Created new chat:', chat);
        }
        ...
        console.error('[CHAT] Error in /direct/:touristId/:guideId', err);
        res.status(500).json({ error: 'Server error' });
    }
});
